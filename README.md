...
| 多类型热重载 | 支持 Spring Bean class、普通 class、MyBatis Mapper XML |
...
- 演示 Spring Bean 方法体热重载。
- 演示普通 Java class 方法体热重载。
...
- `file`：上传的 `.class` 或 `.xml` 文件。
- `type`：可选，支持 `SPRING_BEAN`、`COMMON_CLASS`、`MYBATIS_XML`。
- `beanName`：可选，Spring Bean 热重载时指定 Bean 名称。
...
- `ips`：目标节点 IP 列表。
- `beanName`：Spring Bean 热重载时可选，未传时会尝试根据 class 匹配容器 Bean。
- `taskRemark`：任务备注。
...
| 类型 | 支持范围 | 说明 |
| --- | --- | --- |
| Spring Bean 方法体 | Service、Manager、Component 等容器管理的 Bean | 适合修改已有方法内部逻辑 |
| 普通 Java class | 未被 Spring 容器管理的工具类、常量类等 | 适合修改已有静态方法或普通方法内部逻辑 |
...
- Spring Bean 恢复文件缺少正确的 BeanName 元数据，启动恢复时找不到目标 Bean。
...
4. 执行 Spring Bean、普通 class 或 MyBatis XML 热重载任务。
