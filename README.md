...
基于 Byte Buddy Agent、Redis 发布订阅和关系型数据库任务日志实现的 Spring Boot 集群热重载示例项目。
...
单体服务里热重载一个类比较直接：通过 Byte Buddy Agent 获取 JVM Instrumentation 并重新定义已加载 class；MyBatis Mapper XML 则直接刷新当前 SqlSessionFactory 的配置。
...
5. Redis 发布任务通知。
6. 目标节点从数据库拉取任务和文件，在本机执行 Byte Buddy Agent class 重定义或 MyBatis XML 刷新。
7. 节点把执行结果写回数据库，页面按任务或日志接口查询。

## 可以达到什么目的

- 演示使用 Byte Buddy Agent 和统一的 `CLASS` 类型热重载 Spring 管理类及普通 Java 类的方法体。
...
<p align="center">
  <img src="docs/images/architecture.svg" alt="Byte Buddy Agent 集群热重载整体架构" width="100%">
</p>
...
- JVM class 由 Byte Buddy Agent 调用 Instrumentation 重新定义；MyBatis XML 由 MyBatis 配置刷新逻辑独立处理。
...
│   └── runtime      Byte Buddy Agent class 重定义和 MyBatis XML 刷新
├── entity           数据库表实体
...
- 应用启动时会调用 `ByteBuddyAgent.install()` 获取 Instrumentation；安装失败或当前 JVM 不支持类重定义时，项目会直接启动失败。
- 运行环境应使用完整 JDK 并允许 Attach API。受限容器或禁用动态 Attach 的环境，可在 JVM 启动参数中预先配置 `-javaagent:/path/to/byte-buddy-agent-1.18.8.jar`。
...
单机接口不依赖 Redis 广播和数据库任务表，适合先验证 Byte Buddy Agent 和运行时重载能力。
...
返回内容包括 Agent 是否已安装，以及当前 JVM 是否支持 class redefine/retransform。
...
这些限制主要来自 JVM Instrumentation `redefineClasses`：
...
### Byte Buddy Agent 安装失败怎么办？

项目启动时会主动安装 Agent，并校验当前 JVM 是否支持 `redefineClasses`。安装失败时不会降级运行，以免节点能接收任务却无法执行 class 热重载。

优先检查：

- 是否使用完整 JDK，而不是缺少 Attach API 的精简 JRE。
- 容器安全策略、JVM 参数或操作系统权限是否禁止进程自 Attach。
- 当前 JVM 实现是否支持 class redefine。
- 受限环境可在启动命令中增加 `-javaagent:/path/to/byte-buddy-agent-1.18.8.jar`，让 Instrumentation 在 JVM 启动阶段完成注册。

MyBatis XML 热重载不经过 Byte Buddy Agent，但本项目启动时仍会统一校验 Agent，保证该节点具备完整的 class 热重载能力。
